...
const checkConflict = async (date: string, time: string, attendantId: string): Promise<boolean> => { return false; }; // Verified Obsolete

// ... (POST logic updated above)
...
router.put('/:id', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if ((updates.attendantId || updates.date || updates.time) && merged.status === 'Pendente') {
            // 4a. If Attendant changed or time changed, Re-Check Schedule/Pause Availability (Ideally)
            if (updates.attendantId || updates.time || updates.date) {
                const { data: attendant } = await supabase.from('user').select('*').eq('id', merged.attendant_id).single();
                if (attendant) {
                    const isAvailable = isAttendantWithinSchedule(attendant, merged.date, merged.time, merged.type);
                    if (!isAvailable) {
        ...
        router.post('/', async (req: Request, res: Response) => {
            try {
                ...
                // Manual/Pre-resolved Selection - Verify Schedule & Conflicts Strictly
                    ...
                    // Fetch Attendant for Schedule Check
                    ...
                    // Check Schedule & Pauses (Robust Backend Check)
                ...
                // 3. Conflict Check (Spec 2.B.7) - Range Based
                // Fetch existing appointments for this attendant/date
                const { data: existingAppts } = await supabase
                    .from('appointments')
                    .select('id, attendant_id, date, time, type, status')
                    .eq('date', data.date)
        ...
        router.put('/:id', async (req: Request, res: Response) => {
            ...
            try {
                ...
                if ((updates.attendantId || updates.date || updates.time) && merged.status === 'Pendente') {
                    ...
                    // 4a. Verify Schedule/Pause Availability
                    const { data: attendant } = await supabase.from('user').select('*').eq('id', merged.attendant_id).single();
                    if (attendant) {
