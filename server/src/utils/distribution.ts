...
export const findBestAttendant = async (
    ...
    let sectors = ['Closer']; // Default for Closer-related types
    let roleFilter: string | null = null;

    const isCloserType = ['Ligação Closer', 'Gold Call', 'Reagendamento Closer', 'Upgrade'].includes(type);

    // If it's NOT a Closer type, or if it's an event-specific appointment (like Onboarding), determine the sector from the event
